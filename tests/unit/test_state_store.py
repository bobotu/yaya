...
from yeelight_pro.session.state import StateStore  # noqa: E402
...
def test_command_does_not_publish_before_target_is_observed() -> None:
    store = _state(("light-a", 2, {"p": True}, True))

    batch_id, prepared = store.prepare_batch({"light-a": {"p": False}}, deadline=10.0)
    accepted = store.accept_batch(batch_id)

    assert not prepared.visible_changed
    assert not accepted.visible_changed
    ...
    assert confirmed.changed_node_ids == frozenset({"light-a"})
...
def test_equal_baseline_still_creates_a_hold_against_late_opposite_state() -> None:
    ...
    batch_id, _result = store.prepare_batch({"light-a": {"p": True}}, deadline=10.0)
...
def test_gateway_batch_releases_all_members_together() -> None:
    store = _state(
        ("ordinary", 2, {"p": True, "l": 20}, True),
        ("group", 4, {"p": True, "l": 20}, True),
    )
    batch_id, _result = store.prepare_batch(
    ...
    assert store.nodes["ordinary"].params["l"] == 20
    assert store.nodes["group"].params["l"] == 20

    second = store.apply_groups(_groups(("group", {"l": 80}, None)))

    assert second.changed_node_ids == frozenset({"ordinary", "group"})
...
def test_unrelated_property_publishes_while_target_property_is_held() -> None:
    ...
    batch_id, _result = store.prepare_batch({"light-a": {"l": 80}}, deadline=10.0)
    store.accept_batch(batch_id)
    ...
    assert store.nodes["light-a"].params == {"p": True, "l": 20, "ct": 4000}


def test_deadline_releases_current_raw_without_marking_node_offline() -> None:
    store = _state(("light-a", 2, {"p": True}, True))
    batch_id, _result = store.prepare_batch({"light-a": {"p": False}}, deadline=5.0)
    store.accept_batch(batch_id)
    ...
    assert not expired.visible_changed
...
def test_deadline_publishes_observed_offline_state() -> None:
    ...
    batch_id, _result = store.prepare_batch({"group": {"p": False}}, deadline=5.0)
    store.accept_batch(batch_id)
...
def test_rpc_failure_removes_only_failed_batch_owners() -> None:
    ...
    first_id, _result = store.prepare_batch({"light-a": {"p": True}}, deadline=10.0)
    store.accept_batch(first_id)
    second_id, _result = store.prepare_batch({"light-a": {"l": 80}}, deadline=11.0)
    ...
    assert store.nodes["light-a"].params == {"p": False, "l": 40}
    assert store.has_pending("light-a", ["p"])
...
def test_newer_batch_fences_old_readback_and_timer() -> None:
    store = _state(("light-a", 2, {"p": False}, True))
    first_id, _result = store.prepare_batch({"light-a": {"p": True}}, deadline=5.0)
    store.accept_batch(first_id)
    second_id, _result = store.prepare_batch({"light-a": {"p": False}}, deadline=10.0)
...
def test_a_b_a_supersession_never_exposes_old_b() -> None:
    store = _state(("light-a", 2, {"p": True}, True))
    off_id, _result = store.prepare_batch({"light-a": {"p": False}}, deadline=5.0)
    store.accept_batch(off_id)
    on_id, _result = store.prepare_batch({"light-a": {"p": True}}, deadline=10.0)
...
def test_superseding_one_property_can_release_remaining_old_target() -> None:
    store = _state(("light-a", 2, {"p": False, "l": 20}, True))
    first_id, _result = store.prepare_batch({"light-a": {"p": True, "l": 80}}, deadline=5.0)
    store.accept_batch(first_id)
    store.apply_properties(_props(("light-a", {"p": True}, None)))

    second_id, prepared = store.prepare_batch({"light-a": {"l": 40}}, deadline=10.0)

    assert prepared.ended_batch_ids == (first_id,)
    assert prepared.changed_node_ids == frozenset({"light-a"})
    assert store.nodes["light-a"].params == {"p": True, "l": 20}
...
def test_disconnect_clears_holds_and_exposes_last_observation() -> None:
    store = _state(("light-a", 2, {"p": True}, True))
    batch_id, _result = store.prepare_batch({"light-a": {"p": False}}, deadline=10.0)
    store.accept_batch(batch_id)
    store.apply_properties(_props(("light-a", {"p": True}, None)))
...
def test_randomized_sequence_preserves_observed_visible_values() -> None:
    rng = random.Random(20260710)
    store = _state(("light-a", 2, {"p": False, "l": 10}, True))
    observed: dict[str, set[object]] = {"p": {False}, "l": {10}}
    active_ids: list[int] = []
    ...
    for step in range(500):
        ...
        assert visible["p"] in observed["p"]
        assert visible["l"] in observed["l"]
        assert raw["p"] in observed["p"]
        assert raw["l"] in observed["l"]
